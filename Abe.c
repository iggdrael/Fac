...
//sert à implémenter une variante plus efficace de l'algo de Prim pr calculer sdles arbres couvrants de poids optimal.
...
int eAbeExtraire(){
	...
	if (kbFilE){
		...
		if (kbFileVoiR) sd("renfile",nIndex);
	}
	nCOUT++;
...
void AbeModifier(int sSommet,int iPoidsCourant){//O(1)
	...
	if (kbFileVoiR) sd("enfile",nIndex);
	nCOUT++;
}//AbeModifier
...
void AbePropager(){//O(S)
	...
	if (!kbFilE)
		...
		if (iPere>0){//remonter les plus petits poids en recopiant la paire d'index iFilsDePoidsMin dans la paire d'index iPere.
			...
			//d(iPere);
			DequeEnfiler(iPere);
			if (kbFileVoiR) sd("    réenfile",iPere);
		}
	}//else while
