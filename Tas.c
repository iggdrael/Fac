...
#define knEffectifLiM 512 //32768 nb max d'éléments admissible physiquement dans le tas; k comme constante;n comme entier naturel.
...
#define knPoidsMaX kE6-1 //poids maximal associé à chaque objet du tas.
...
void TasActualiser(int uElement,int nPoids){//O(lg(nEffectifMax))
	if (bVerifieR) Assert1("TasActualiser1",bTasContient(uElement));
...
void TasAllouer(int nEffectifMax){//O(1)
	...
	if (kbParler) Appel0((nEffectifMax>0)?"Allocation d'un nouveau tas":"Restitution du tas précédemment alloué");
		if (bVerifieR) Assert2("TasAllouer",bTasAmorceR,bCroit(0,nEffectifMax,knEffectifMaX));
...
int bTasContient(int uObjet){//O(1)
	...
	int bContient;
	if (bVerifieR) Assert3("bTasContient",bTasAmorceR,bTasOuverT,bCroit(1,uObjet,knEffectifMaX));
...
void TasDescendrE(int uObjet){//O(1)
	...
	if (bVerifieR) Assert1("TasDescendrE1",bCroit(1,uObjet,knEffectifMaX));
	uPere=-pTaS->iPlace[uObjet];
	//tee("uObjet,Place>0",uObjet,uPere);
	if (bVerifieR) Assert1("TasDescendrE2",bCroit(1,uPere,pTaS->nCardinal));
	uFils=2*uPere;
...
int eTasExtraire(){//O(lg(N))
	...
	int nDernier;
	//Appel0("eTasExtraire");
	if (bVerifieR) Assert1("eTasExtraire1",pTaS->nVolume>0);
	...
	if (bVerifieR) Assert2("eTasExtraire2",bCroit(1,eExtraire,knEffectifMaX),pTaS->nVolume>=0);
	if (bVerifieR) TasPlausiblE("eTasExtraire");
	//Appel1("eTasExtraire");
	return(eExtraire);
}//eTasExtraire
...
void TasInsererValuer(int uObjet,int nPoids){
	...
	if (bVerifieR) Assert3("TasInsererValuer",bTasAmorceR,bTasOuverT,bCroit(1,uObjet,knEffectifMaX));
	nCOUT++;
...
void TasMonteR(int uObjet){//O(1)
	int uFils,nPere;
	if (bVerifieR) Assert3("TasMonteR1",bTasAmorceR,bTasOuverT,uObjet>0);
	...
	if (bVerifieR) Assert2("TasMonteR2",bCroit(1,uFils,pTaS->nVolume),bCroit(0,nPere,uFils-1));
...
void TasRazer(){//O(nCardinalMax)
	int uE;
	if (bVerifieR) Assert2("TasRazer",bTasAmorceR,bTasOuverT);
...
void TasSatureR(){//O(knEffectifMaX)
	...
	int uE;
	if (bVerifieR) Assert2("TasSatureR",bTasAmorceR,bTasOuverT);
	for (uE=1;uE<=knEffectifMaX;uE++)
		pTaS->nObjet[uE]=uE;
	for (uE=1;uE<=knEffectifMaX;uE++)
		pTaS->iPlace[uE]=-uE;
...
int eTasSommet(){//O(1)
	...
	int eSommet;
	if (bVerifieR) Assert2("eTasSommet1",bTasAmorceR,bTasOuverT);
	if (bTasVide())
		eSommet=0;
	else eSommet=pTaS->nObjet[1];
	if (bVerifieR) Assert2("eTasSommet2",bCroit(0,eSommet,knEffectifMaX),(eSommet==0)==bTasVide());
	nCOUT++;
	return(eSommet);
}//eTasSommet
...
int bTasVide(){//O(1)
	int bVide;
	if (bVerifieR) Assert2("bTasVide",bTasAmorceR,bTasOuverT);
	bVide=(pTaS->nVolume==0);
	nCOUT++;
	return(bVide);
}//bTasVide

void TasVerifier(int bVerifier){
	Assert1("TasVerifier",bBool(bVerifier));
	bVerifieR=bVerifier;
}//TasVerifier
