...
#define kuDequeCardinalMaX (k2E12-1) //nb max de sommets admissible ds la deque; k pour constante;u pour Un ou plus, kP10=2**10=1024). 
...
void DequeDepiler(){//O(1)
	int iDepiler=0;
	if (!bDequeVide())
		iDepiler=sDequeDepiler();
}//DequeDepiler

int sDequeDepiler(){//O(1)
	int sDepiler=0;
	Assert5("sDequeDepiler0",bDequeAmorceR,bDequeOuvertE,bCroit(1,nCardMaX,kuDequeCardinalMaX),bCroit(1,sLasT,nCardMaX),tModePileOuFilE>=0);
	tModePileOuFilE=+1;
	if (nCarD>0) {
	...
	if (! ( bCroit(1,sDepiler,kuDequeCardinalMaX) && nCarD>=0) )
		d3(sDepiler,kuDequeCardinalMaX,nCarD);
	Assert2("sDequeDepiler1",bCroit(1,sDepiler,kuDequeCardinalMaX),nCarD>=0);
	return(sDepiler);
}//sDequeDepiler
...
int sDequeFond(){//O(1)
	int sFond=0;
	Assert5("sDequeFond",bDequeAmorceR,bDequeOuvertE,bCroit(1,nCardMaX,kuDequeCardinalMaX),bCroit(1,nCarD,nCardMaX),tModePileOuFilE>0);
	if (nCarD>0)//ce test évite de retourner une valeur flottante (copie de LasT) au lieu d'une constante (0) quand les assertions sont désactivées
		sFond=sLasT;
	Assert1("sDequeFond",bCroit(1,sFond,nCardMaX));
	return(sFond);
}//sDequeFond
...
void DequeTESTER(int iTest){
	...
	Appel0(sC2("DequeTester,test n°",sEnt(iTest)));
		switch (iTest) {
			case 1:
				DequeAllouer(4);
					DequeScruter("Deque allouée et vide, censée être utilisée en mode pile.");
					DequeEmpiler(1);
					DequeEmpiler(2);
					DequeEmpiler(3);
					DequeVoir("");
					DequeScruter("Deque pleine:");
					DequeVoir("");
					sSource=sDequeDepiler();
					sd("Sommet dépilé",sSource);
					DequeScruter("Deque résultante:");
					sSource=sDequeDepiler();
					sd("Sommet dépilé",sSource);
					DequeScruter("Deque finale:");
				DequeAllouer(0);
				DequeAllouer(12);
					DequeScruter("Deque allouée et vide, censée être utilisée en mode file.");
					DequeEnfiler(1);
					DequeEnfiler(2);
					DequeEnfiler(3);
					DequeVoir("");
			...
			case 2:
				DequeAllouer(4);
					DequeScruter("Apres DequeAllouer(4)");
					DequeEmpiler(2);
					DequeScruter("Apres DequeEmpiler(2)");
					DequeEmpiler(4);
					DequeScruter("Apres DequeEmpiler(4)");
					DequeEmpiler(3);
					DequeScruter("Apres DequeEmpiler(3)");
					sSource=sDequeDepiler();
					sd("Sommet dépilé",sSource);
					DequeScruter("Apres DequeDepiler()");
					sSource=sDequeDepiler();
					sd("Sommet dépilé",sSource);
					DequeScruter("Apres DequeDepiler()");
				DequeAllouer(0);
			...
			case 3:
				DequeAllouer(4);
					DequeScruter("Apres DequeAllouer(4)");
					DequeEnfiler(2);
					DequeScruter("Apres DequeEnfiler(2)");
					DequeEnfiler(4);
					DequeScruter("Apres DequeEnfiler(4)");
					DequeEnfiler(3);
					DequeScruter("Apres DequeEnfiler(3)");
					sSource=sDequeDefiler();
					sd("Sommet défilé",sSource);
					DequeScruter("Apres DequeDefiler()");
					sSource=sDequeDefiler();
					sd("Sommet défilé",sSource);
					DequeScruter("Apres DequeDefiler()");
				DequeAllouer(0);
...
void DequeScruter(char *sMessage){//O(nCardMaX)
	int uS;
	Assert3("DequeScruter",bDequeAmorceR,bDequeOuvertE,sMessage!=0);
	printf("%s:\t",sMessage);
...
void DequeVoir(char *sMessage){//O(taille courante de la deque)
	Assert1("DequeVoir",tModePileOuFilE!=0);
	if (tModePileOuFilE>0)
		DequeVoirPilE(sMessage);
	else DequeVoirFilE(sMessage);
}//DequeVoir
...
void DequeVoirFilE(char *sMessage){//O(taille courante de la deque)
	...
	int nCardinal,tMode,uS;
	int nFile[1+nCardMaX];
	Assert4("DequeVoirFilE",bDequeAmorceR,bDequeOuvertE,sMessage!=0,bTrit(tModePileOuFilE));
	printf("File %s%s: ",sMessage,sDomaine(1,nDequeCardinal()));
	//DequeScruter("AVANT");
		tMode=tModePileOuFilE;
			for (nCardinal=nDequeCardinal(),uS=1;tModePileOuFilE=0,uS<=nCardinal;uS++)
				nFile[uS]=sDequeDefiler();
			//afficher la file stockée dans nFile[]
				for (uS=nCardinal;uS>=1 || bPrint("");uS--)
					printf("%s%s",sEnt(nFile[uS]),(uS>1)? ",":".");
			//reconstituer la deque (uS croît ctrairt à DequeVoirPilE)
				for (uS=1;tModePileOuFilE=0,uS<=nCardinal;uS++)
...
void DequeVoirPilE(char *sMessage){//O(taille courante de la deque)
	...
	printf("Pile %s%s: ",sMessage,sDomaine(1,nDequeCardinal()));
	//DequeScruter("AVANT");
		tMode=tModePileOuFilE;
			for (nCardinal=nDequeCardinal(),tModePileOuFilE=0,uS=1;uS<=nCardinal;uS++)
				nPile[uS]=sDequeDepiler();
			//afficher la pile stockée dans nPile[]
				for (uS=nCardinal;uS>=1 || bPrint("");uS--)
					printf("%s%s",sEnt(nPile[uS]),(uS>1)?",":".");
			//reconstituer la deque (uS décroît ctrairt à DequeVoirFilE)
				for (uS=nCardinal;tModePileOuFilE=0,uS>=1;uS--)
